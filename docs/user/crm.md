...
#### E. Task Privacy & Scoping (Public vs. Private)
To balance collaborative commercial deal progression with individual operator privacy:
- **Opportunity-Linked Tasks (Public)**: Tasks attached to an Opportunity (`opportunityId IS NOT NULL`) represent shared commercial deliverables and are visible across the entire team to anyone with CRM read access.
- **Standalone Tasks (Private)**: Tasks not attached to an Opportunity (`opportunityId IS NULL`) are personal action items. They are strictly private and visible only to the assigned user (`assignedToUserId`), the creator (`createdById`), and system administrators. Unauthorized users cannot view, query, update, complete, or delete another operator's private tasks.
- **Automatic Assignment**: Creating a standalone private task without explicitly designating an assignee automatically assigns the task to the creator.

#### F. In-Line Task Completion
...
#### G. Operations Dashboard Tasks Widget
The homepage dashboard (`/`) includes the **Tasks** widget (`DashboardTasksWidget`):
- **My Open Tasks**: Displays active CRM tasks assigned to you, with overdue dates flagged in red.
