import { convertAmount, HOME_CURRENCY, MissingExchangeRateError } from './currency';
...
describe('currency conversion', () => {
  ...
  it('should convert from HOME_CURRENCY to another currency', () => {
  ...
  it('should convert from another currency to HOME_CURRENCY', () => {
  ...
  it('should convert between two non-home currencies', () => {
    // 100 USD -> EUR (100 / 1.10 = 90.909...) -> GBP (90.909... * 0.85 = 77.2727...)
    // 100 / 1.10 * 0.85 = 77.27
    expect(convertAmount(100, 'USD', 'GBP', rates)).toBe(77.27);
  });

  it('should throw MissingExchangeRateError if a rate is missing', () => {
    expect(() => convertAmount(100, 'EUR', 'JPY', rates)).toThrow(MissingExchangeRateError);
    expect(() => convertAmount(100, 'JPY', 'EUR', rates)).toThrow(MissingExchangeRateError);
    expect(() => convertAmount(100, 'USD', 'JPY', rates)).toThrow(MissingExchangeRateError);
  });
  ...
  it('should handle string rates correctly', () => {
    ...
    expect(convertAmount(100, 'EUR', 'USD', stringRates)).toBe(110);
  });
  ...
  it('should handle rounding based on currency decimals', () => {
    ...
    expect(convertAmount(100, 'EUR', 'JPY', ratesWithJpy)).toBe(14056);
  });
});
