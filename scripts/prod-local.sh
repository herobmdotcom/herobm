...
eval "env ENV_FILE='$ENV_FILE' PORT=$API_PORT PIPELINE_LOG_DIR='$LOG_DIR' $ENV_EXPORTS npm run start:prod -w apps/api &"
API_PID=$!

# Start FE in foreground
eval "env ENV_FILE='$ENV_FILE' API_URL='http://localhost:$API_PORT' $ENV_EXPORTS npm run start -w apps/ops-portal -- -p $FE_PORT"
