// Purchase Order — herobm report template
...
#show: doc => conf(title: "PURCHASE ORDER", doc)
...
#grid(
  ...
  gutter: 10pt,
  [
    #text(12pt, weight: "bold", fill: theme.primaryColor)[#data.header.orderNumber] \
...
#grid(
  ...
  [
    #let ref = if "supplierReference" in data.header and data.header.supplierReference != "" and data.header.supplierReference != none { data.header.supplierReference } else if "referenceNumber" in data.header and data.header.referenceNumber != "" and data.header.referenceNumber != none { data.header.referenceNumber } else if "customerOrderNumber" in data.header and data.header.customerOrderNumber != "" and data.header.customerOrderNumber != none { data.header.customerOrderNumber } else { "—" }
    #grid(
      columns: (auto, 1fr),
      row-gutter: 8pt,
      column-gutter: 12pt,
      text(9pt, weight: "bold", fill: theme.mutedColor)[Date:], data.header.orderDate,
      text(9pt, weight: "bold", fill: theme.mutedColor)[Reference:], ref,
...
#text(8pt, fill: theme.mutedColor, style: "italic")[
  Please process this purchase order according to our agreed terms and conditions.
]
