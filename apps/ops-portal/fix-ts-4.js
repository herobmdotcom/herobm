...
const startIdx = content.indexOf('  const ratesWithBase = useMemo(() => {');
const endString = '        if (acct.isGroup) {';
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  const coaTree = useMemo(() => {
    ...
    content = content.substring(0, startIdx) + replacement + content.substring(endIdx + endString.length);
}
