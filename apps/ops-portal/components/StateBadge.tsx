...
export default function StateBadge({ state }: StateBadgeProps) {
  ...
  if (!s || !t.has(s as Parameters<typeof t.has>[0])) {
    return <span className={`badge badge-legacy`}>{String(state || 'unknown').replace(/_/g, ' ')}</span>;
  }
...
export function StateName({ state }: StateBadgeProps) {
  const t = useTranslations('common.states');
  const s = String(state).toLowerCase();
  if (!s || !t.has(s as Parameters<typeof t.has>[0])) return <span>{String(state || 'unknown').replace(/_/g, ' ')}</span>;
