  ...
  return (
    <div className={`space-y-4 text-sm text-[var(--text-secondary)] leading-relaxed select-text font-sans ${className}`}>
      {chunks.map((chunk, index) => {
        ...
        // Parse alert blocks into structured segments so alert containers are rendered as React JSX
        // and all markdown contents are safely rendered with html: false
        ...
        const alertRegex = /^[ \t]*>[ \t]*\[!(NOTE|IMPORTANT|WARNING|TIP|CAUTION|INFO|DANGER)\][^\n]*\n((?:[ \t]*>.*(?:\n|$))*)/gim;
        let lastIndex = 0;
        let match: RegExpExecArray | null;

        while ((match = alertRegex.exec(normalized)) !== null) {
          if (match.index > lastIndex) {
            const textPart = normalized.substring(lastIndex, match.index).trim();
            if (textPart) {
              segments.push({
                type: 'markdown',
                content: textPart,
        ...
        return (
          <React.Fragment key={index}>
            {segments.map((seg, segIdx) => {
              if (seg.type === 'alert') {
                const typeUpper = seg.alertType || 'NOTE';
                ...
                const alertHtml = md.render(seg.content);

                return (
                  <div
                    key={segIdx}
                    className={`my-4 p-3.5 rounded-xl border ${borderClass} text-xs leading-relaxed not-italic`}
                  >
                    <div className={`font-bold uppercase tracking-wider text-[11px] mb-2 flex items-center gap-1.5 ${badgeClass}`}>
                      <span className="material-symbols-outlined text-[16px]">{icon}</span> {typeUpper}
                    </div>
                    <div
                      className="space-y-1.5 [&>p]:my-1 [&>ul]:list-disc [&>ul]:pl-4 [&>ul>li]:my-0.5 [&>ol]:list-decimal [&>ol]:pl-4 [&_code]:bg-[var(--bg-secondary)] [&_code]:text-[var(--accent)] [&_code]:border [&_code]:border-[var(--border)] [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:rounded [&_code]:font-mono [&_code]:text-[11px] [&_strong]:font-bold [&_strong]:text-[var(--text-primary)] [&_a]:font-semibold [&_a]:text-[var(--accent)] [&_a]:underline [&_a]:underline-offset-2 hover:[&_a]:opacity-80"
