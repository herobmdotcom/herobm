...
interface EditTaskSlideOverProps {
  isOpen: boolean;
  onClose: () => void;
  task: api.ProjectTaskResponseDto | null;
  tasks: api.ProjectTaskResponseDto[];
  onSubmit: (taskId: string, dto: api.UpdateProjectTaskDto) => Promise<void>;
}

export function EditTaskSlideOver({
  isOpen,
  onClose,
  task,
  tasks,
  onSubmit,
}: EditTaskSlideOverProps) {
    ...
    if (isOpen && task) {
      setForm({
        ...
        plannedEndDate: task.plannedEndDate
          ? new Date(task.plannedEndDate).toISOString().slice(0, 10)
          : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!task || !form.taskCode.trim() || !form.name.trim()) return;

    setSubmitting(true);
    try {
      await onSubmit(task.projectTaskId, {
  ...
  const availableParentTasks = tasks.filter(
    (tk) => tk.projectTaskId !== task?.projectTaskId,
  );
      ...
      footer={
        <div className="flex items-center justify-end gap-3 w-full">
          ...
          <Button
            ...
            title={isCompleted ? 'Completed tasks cannot be modified' : undefined}
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              value={form.taskCode}
              onChange={(e) =>
                setForm((p) => ({ ...p, taskCode: e.target.value }))
              }
            />
          </div>
          <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, plannedStartDate: e.target.value }))
              }
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, plannedEndDate: e.target.value }))
              }
            />
          </div>
        </div>
        ...
        <div className="space-y-3 pt-2">
          <label className="flex items-center gap-2.5 cursor-pointer">
            <input
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, isMilestone: e.target.checked }))
              }
            />
          ...
          <label className="flex items-center gap-2.5 cursor-pointer">
            <input
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, isBillable: e.target.checked }))
              }
            />
            <span className="text-sm text-[var(--text-primary)]">
              {t('labels.isBillable')}
