...
describe('OpportunitiesContent', () => {
  ...
  it('applies high-contrast active styling to the selected view mode toggle', async () => {
    render(<OpportunitiesContent />);
    await waitFor(() => {
      expect(screen.getByText('Deal Alpha')).toBeInTheDocument();
    });

    const kanbanBtn = screen.getAllByRole('button', { name: /kanban/i })[0];
    const listBtn = screen.getAllByRole('button', { name: /list/i })[0];

    // Default: Kanban active, List inactive
    expect(kanbanBtn.className).toContain('dark:!bg-white/20');
    expect(kanbanBtn.className).toContain('dark:!text-white');
    expect(listBtn.className).toContain('!bg-transparent');

    // Switch to List
    fireEvent.click(listBtn);
    const updatedListBtn = screen.getAllByRole('button', { name: /list/i })[0];
    const updatedKanbanBtn = screen.getAllByRole('button', { name: /kanban/i })[0];
