...
describe('OpportunityKanbanCard', () => {
  ...
  it('renders target close date without a calendar icon', () => {
    render(
      <OpportunityKanbanCard
        opportunity={mockOpp}
        stages={STAGES}
  ...
  it('renders opportunity owner when present', () => {
    const oppWithOwner: OpportunityResponseDto = {
      ...mockOpp,
      owner: { displayName: 'Sarah Connor' },
    } as unknown as OpportunityResponseDto;

    render(
      <OpportunityKanbanCard
        opportunity={oppWithOwner}
        stages={STAGES}
        onMoveStage={jest.fn()}
      />,
    );
