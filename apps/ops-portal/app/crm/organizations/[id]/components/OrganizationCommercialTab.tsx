...
import type { OrganizationResponseDto } from '@herobm/sdk';
...
export function OrganizationCommercialTab({ organizationId, organization }: OrganizationCommercialTabProps) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- DTO typing
  const customer = organization?.customers && (organization.customers as any[])[0];
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- DTO typing
  const supplier = organization?.suppliers && (organization.suppliers as any[])[0];
    ...
    <div className="flex flex-col gap-8 max-w-6xl">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-6 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--text-muted)]">
                Sales accounts, credit terms, and orders linked to this organization
              </p>
            </div>
          </div>
          {customer ? (
            ...
            <Button asChild size="sm" variant="primary">
              <Link
                href={`/customers/new?organizationId=${organizationId}&name=${encodeURIComponent(organization?.name || '')}`}
              >
        ...
        {customer ? (
          ...
          <div className="py-8 text-center text-sm text-[var(--text-muted)]">
            This Organization is not currently registered as a Customer. Click above to create a Customer account linked to this organization profile.
          </div>
        )}
      </div>
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-[var(--border)]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--text-muted)]">
                Procurement accounts, payment settings, and purchase orders linked to this organization
              </p>
            </div>
          </div>
          {supplier ? (
            ...
            <Button asChild size="sm" variant="primary">
              <Link
                href={`/suppliers/new?organizationId=${organizationId}&name=${encodeURIComponent(organization?.name || '')}`}
              >
        ...
        {supplier ? (
          <div className="flex flex-col gap-6 mt-6">
            ...
            <div className="mt-2">
              <DetailTabGrid<PurchaseOrderRow>
                ...
                gridKey={`organization-purchase-orders-${supplier.vendorId}`}
                fetchAll
                rowIdField="id"
          ...
          <div className="py-8 text-center text-sm text-[var(--text-muted)]">
            This Organization is not currently registered as a Supplier. Click above to create a Supplier account linked to this organization profile.
          </div>
        )}
      </div>
    </div>
