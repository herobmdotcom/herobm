...
import type { OrganizationLinkResponseDto } from '@herobm/sdk';
...
export function OrganizationHierarchyTab({ organizationId }: OrganizationHierarchyTabProps) {
  const [links, setLinks] = useState<OrganizationLinkResponseDto[]>([]);
  ...
  const loadLinks = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.organizationsControllerGetLinks(organizationId);
      setLinks((res.data as unknown as OrganizationLinkResponseDto[]) || []);
    } catch (err) {
      reportError(err, 'OrganizationHierarchyTab:loadLinks');
  ...
  const handleAddLink = async () => {
    ...
    setSaving(true);
    try {
      await api.organizationsControllerAddLink(organizationId, {
        targetOrganizationId: selectedOrganization.organizationId,
      ...
      reportError(err, 'OrganizationHierarchyTab:handleAddLink');
  ...
  const handleRemoveLink = async (linkId: string, partnerName: string) => {
    ...
    try {
      await api.organizationsControllerRemoveLink(organizationId, linkId);
      toast.success('Corporate link removed');
      await loadLinks();
    } catch (err) {
      reportError(err, 'OrganizationHierarchyTab:handleRemoveLink');
      toast.error(getErrorMessage(err));
    }
  };

  // Categorize links from this organization's perspective
  const categorized = links.map((link) => {
    const isSource = link.sourceOrganizationId === organizationId;
    const partner = isSource ? link.targetOrganization : link.sourceOrganization;
    ...
    return {
      linkId: link.linkId,
      partnerId: partner?.organizationId || (isSource ? link.targetOrganizationId : link.sourceOrganizationId),
      partnerName: partner?.name || 'Unknown Organization',
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-xl bg-[var(--bg-card)] border border-[var(--border)]">
        ...
        <div className="flex items-center gap-2">
          <Button asChild size="sm" variant="secondary">
            <Link href={`/crm/map?organizationId=${organizationId}`}>
              View Network Graph
            </Link>
          </Button>
          <Button
        ...
        <div className="p-5 rounded-xl bg-[var(--bg-card)] border border-[var(--accent)]/30 shadow-sm flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                <option value="parent_company">Parent Company (target is parent of this organization)</option>
                <option value="subsidiary">Subsidiary (target is owned by this organization)</option>
          ...
          {categorized.map((item) => (
            ...
            >
              <div className="flex flex-col gap-1 min-w-0">
                <div className="flex items-center gap-2">
                  <Link
                    href={`/crm/organizations/${item.partnerId}`}
