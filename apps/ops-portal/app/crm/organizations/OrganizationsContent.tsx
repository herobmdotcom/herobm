...
export default function OrganizationsContent() {
  ...
  useEffect(() => {
    const loadUsers = async () => {
      try {
        ...
        reportError(err, 'OrganizationsContent - loadUsers');
  ...
  const endpoint = useMemo(() => {
    ...
    if (selectedOwner === 'my') {
      return currentUserId ? `/api/organizations?ownerId=${encodeURIComponent(currentUserId)}` : '/api/organizations';
    }
    return `/api/organizations?ownerId=${encodeURIComponent(selectedOwner)}`;
  ...
  return (
    <DataGrid
      ...
      rowIdField="organizationId"
      rowHref={(row) => `/crm/organizations/${row.organizationId || row.actorId}`}
