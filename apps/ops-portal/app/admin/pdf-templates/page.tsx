...
export default function ReportingPage() {
  ...
  const handleRowClicked = useCallback((row: any) => {
    router.push(`/admin/pdf-templates/${row.id}`);
  }, [router]);
    ...
    <>
      <>
            <DataGrid
              ...
              pageTitle={t('title')}
              headerActions={
                <Link href="/admin/pdf-templates/new" className="px-4 py-2 text-sm font-bold rounded-lg transition-all bg-[#006b5c] text-white hover:brightness-110 ml-2 lg:ml-0 whitespace-nowrap">
                  {t('grid.createButton')}
                </Link>
              }
            />
