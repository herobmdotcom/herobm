...
export default function NewPurchaseInvoicePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(`/purchase-invoices/${invoice.invoiceId}`);
    } catch (err: unknown) {
      ...
      <DetailsLayout
        header={
          <EntityHeader
            ...
            onBack={() => router.push(initialPurchaseOrderId ? `/purchase-orders/${initialPurchaseOrderId}` : '/purchase-invoices')}
            actions={
              <>
                <button
                  className="btn btn-secondary btn-sm"
                  onClick={() => router.push(initialPurchaseOrderId ? `/purchase-orders/${initialPurchaseOrderId}` : '/purchase-invoices')}
                  disabled={submitting}
                >
                  {tCommon('cancel')}
                </button>
            ...
            <table className="table-lines">
              ...
              <tbody>
                ...
                <tr style={{ borderTop: '2px solid var(--border)' }}>
                  <td colSpan={4} style={{ textAlign: 'right', fontWeight: 600, color: 'var(--text-muted)' }}>
                    {tCommon('subtotal')}
                  </td>
                  <td style={{ textAlign: 'right', fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>
                ...
                <tr>
                  ...
                  <td style={{ textAlign: 'right' }}>
                    <input
                      ...
                      onBlur={(e) => {
                        const val = parseFloat(e.target.value);
                        if (!isNaN(val)) setTaxAmountInput(val.toFixed(2));
                      }}
                    />
                  </td>
                  <td></td>
                ...
                <tr style={{ backgroundColor: 'rgba(59,130,246,0.02)' }}>
                  <td colSpan={4} style={{ textAlign: 'right', fontWeight: 700, fontSize: 13, color: 'var(--text-primary)' }}>
                    {tCommon('total')}
                  </td>
