...
#grid(
  ...
  [
    #grid(
      ...
      column-gutter: 20pt,
      align: (left, right),
      [Subtotal:], [#data.header.currencyCode #format_decimal(data.summary.subtotal)],
      [Total Tax:], [#data.header.currencyCode #format_decimal(data.summary.totalTax)],
      
      grid.cell(colspan: 2)[#line(length: 100%, stroke: 1pt + luma(230))],
      
      text(12pt, weight: "bold")[Total:], 
      text(12pt, weight: "bold", fill: rgb("#1e3a5f"))[#data.header.currencyCode #format_decimal(data.summary.totalAmount)],
    )
  ]
)
