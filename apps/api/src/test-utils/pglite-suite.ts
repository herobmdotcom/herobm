...
import { createMemoryDb } from '../drizzle/drizzle.module';
 ...
 */
export function setupPgliteSuite(): PgliteTestContext {
  const context = {
    _db: null as any,
    _client: null as any,
    get db() {
      if (!this._db) throw new Error('PGLite context.db accessed before beforeAll');
      return this._db;
    },
    get client() {
      if (!this._client) throw new Error('PGLite context.client accessed before beforeAll');
      return this._client;
    },
  };
