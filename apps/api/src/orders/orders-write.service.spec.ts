...
const GST_DEFAULT = { gstCategoryId: 'gst-default', code: 'GST', rate: '10', isDefault: true };
const GST_EXEMPT = { gstCategoryId: 'gst-exempt', code: 'EXE', rate: '0', isDefault: false };
const GST_ZERO = { gstCategoryId: 'gst-zero', code: 'ZR', rate: '0', isDefault: false };
...
describe('OrdersWriteService', () => {
  ...
  describe('line amount computation', () => {
    const compute = (qty: string, price: string, disc: string, gstRate: number) =>
      (OrdersWriteService.prototype as any).computeLineAmount.call(null, qty, price, disc, gstRate);
  ...
  describe('create', () => {
    ...
    function setupCreate(opts?: { gstPosition?: string; disc?: string; productGst?: string; currency?: string }) {
    ...
    it('should use zero-rated GST for zero-rated product', async () => {
      setupCreate({ gstPosition: 'taxable', productGst: 'Zero Rated Products' });
      await service.create(validDto, 'admin');
    ...
    it('should reject unknown customer', async () => {
      ...
      await expect(service.create(validDto, 'admin')).rejects.toThrow(BadRequestException);
    });
  ...
  describe('update', () => {
    ...
    it('should reject update on invoiced order', async () => {
      ...
      await expect(service.update('uuid-001', { name: 'Test' }, 'admin')).rejects.toThrow(BadRequestException);
    });

    it('should reject update on cancelled order', async () => {
      setupForUpdate('cancelled');
      await expect(service.update('uuid-001', { notes: 'Test' }, 'admin')).rejects.toThrow(BadRequestException);
    });

    it('should throw NotFoundException for unknown order', async () => {
      mockSelectChain({ 1: [] });
      await expect(service.update('NOPE', {}, 'admin')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('changeState', () => {
    ...
    it.each([
      ...
      ['quoted', 'confirmed'], ['quoted', 'draft'], ['quoted', 'cancelled'],
      ['confirmed', 'picking'], ['confirmed', 'cancelled'],
      ['picking', 'shipped'], ['picking', 'confirmed'],
      ['shipped', 'invoiced'], ['cancelled', 'draft'],
    ...
    it.each([
      ['draft', 'shipped'], ['draft', 'invoiced'], ['draft', 'picking'], ['draft', 'confirmed'],
      ['confirmed', 'quoted'], ['shipped', 'draft'],
      ['invoiced', 'draft'], ['invoiced', 'cancelled'],
    ])('should reject transition %s → %s', async (from, to) => {
      setupWithState(from);
      await expect(service.changeState('uuid-001', to, 'admin')).rejects.toThrow(BadRequestException);
    });
  ...
  describe('addLine', () => {
    const lineDto = { productId: 'PROD-001', quantity: '5', pricePerUnit: '12.00' };
    ...
    function setupForAddLine(orderState: string, maxLineNumber = 0) {
      mockSelectChain({
        ...
        5: [{ productId: 'PROD-001', gstCategory: '9% GST' }],      // resolveGstForLine → lookupProduct
      });
    ...
    it('should use per-line GST override when provided', async () => {
      setupForAddLine('draft');
      await service.addLine('uuid-001', { ...lineDto, gstCategoryId: 'gst-exempt' }, 'admin');
    ...
    it('should reject adding to an invoiced order', async () => {
      ...
      await expect(service.addLine('uuid-001', lineDto, 'admin')).rejects.toThrow(BadRequestException);
    });
    ...
    it('should use zero-rate for zero-rated product', async () => {
      mockSelectChain({
        ...
        5: [{ productId: 'PROD-ZR', gstCategory: 'Zero Rated Products' }],
      });
      const txInsertQb = createMockQueryBuilder([{ salesOrderLineId: 'line-1', lineNumber: 1 }]);
      mockDb.transaction = jest.fn().mockImplementation(async (cb: any) => {
        const tx = createMockTx();
        tx.insert = jest.fn().mockReturnValue(txInsertQb);
        return cb(tx);
      ...
      await service.addLine('uuid-001', { ...lineDto, productId: 'PROD-ZR' }, 'admin');
      expect(mockGstService.getByCode).toHaveBeenCalledWith('ZR');
    });
  });
  ...
  describe('updateLine', () => {
    ...
    it('should update line quantity on a draft order', async () => {
      ...
      const result = await service.updateLine('uuid-001', 'line-001', { quantity: '20' }, 'admin');
      expect(result).toHaveProperty('salesOrderLineId', 'line-001');
    });

    it('should resolve GST category for recomputation', async () => {
      setupForUpdateLine('draft');
      await service.updateLine('uuid-001', 'line-001', { quantity: '20' }, 'admin');
  ...
  describe('removeLine', () => {
    ...
    it('should remove a line from a draft order', async () => {
      ...
      await expect(service.removeLine('uuid-001', 'line-001', 'admin')).resolves.toBeUndefined();
    });
    ...
    it('should reject removal from invoiced order', async () => {
      ...
      await expect(service.removeLine('uuid-001', 'line-001', 'admin')).rejects.toThrow(BadRequestException);
    });
