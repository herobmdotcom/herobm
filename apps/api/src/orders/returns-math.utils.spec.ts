...
describe('Returns Math Logic (Pure)', () => {
  ...
  it('should restrict returning quantity less than or equal to 0', () => {
    expect(() => validateReturnQuantity(0, 5, 0, 1)).toThrow(
      BadRequestException,
    );
    expect(() => validateReturnQuantity(-1, 5, 0, 1)).toThrow(
      BadRequestException,
    );
  });

  it('should restrict returning more than the original quantity', () => {
    expect(() => validateReturnQuantity(6, 5, 0, 1)).toThrow(
      BadRequestException,
    );
  });
  ...
  it('should restrict returning more than the remaining permissible quantity', () => {
    ...
    expect(() => validateReturnQuantity(2, 5, 4, 1)).toThrow(
      BadRequestException,
    );
  });
