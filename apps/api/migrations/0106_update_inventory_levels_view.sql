CREATE OR REPLACE VIEW herobm_core.inventory_levels AS
 SELECT gen_random_uuid() AS inventory_level_id,
    ...
    COALESCE((( SELECT COALESCE(sum(b.quantity), 0::numeric) AS "coalesce"
          ...
          WHERE (sol.product_id = p.product_id OR b.product_id = p.product_id) AND sol.fulfillment_location_id = l.location_id AND b.state_code = 'received_reserved'::text)) + 
    ...
    (( SELECT COALESCE(sum((sol.quantity - COALESCE(( SELECT sum(sop.quantity) AS sum
                   FROM herobm_core.sales_order_picks sop
                  WHERE sop.sales_order_line_id = sol.sales_order_line_id), 0::numeric)) * pc.quantity), 0::numeric) AS "coalesce"
           FROM herobm_core.sales_order_lines sol
             JOIN herobm_core.sales_orders so ON so.sales_order_id = sol.sales_order_id
          ...
          WHERE pc.child_product_id = p.product_id AND kit_p.product_type = 'non-stock' AND kit_p.structure_type = 'kit' AND sol.fulfillment_location_id = l.location_id AND (so.state_code = ANY (ARRAY['confirmed'::text, 'picking'::text, 'partially_picked'::text, 'packed'::text, 'partially_dispatched'::text])))), 0::numeric) AS quantity_committed,
